...
describe('CachedPlaceLookup', () => {
  ...
  it('makes a new place when none exists', async () => {
    ...
    const place = await lookup.getPlace('some id', undefined);
    expect(place.id).toEqual('some id');
  });

  it('returns the existing place when one exists', async () => {
    const lookup = new CachedPlaceLookup(10);
    const place1 = await lookup.getPlace('some id', undefined);
    const place2 = await lookup.getPlace('some id', undefined);
    expect(place1).toBe(place2);
  });

  it(`makes one place on repeated synchronous place creation`, async () => {
    const lookup = new CachedPlaceLookup(10);
    const promise1 = lookup.getPlace('some id', undefined);
    const promise2 = lookup.getPlace('some id', undefined);
  ...
  it('adds a new place with updatePlace', async () => {
    ...
    expect(await lookup.getPlace('some id', undefined)).toBe(place);
  });

  it('replaces an existing place with updatePlace', async () => {
    const lookup = new CachedPlaceLookup(10);
    const oldPlace = await lookup.getPlace('some id', undefined);
    const newPlace = makeFakePlace({id: 'some id'});
    lookup.updatePlace(newPlace);
    expect(await lookup.getPlace('some id', undefined)).toBe(newPlace);
    expect(await lookup.getPlace('some id', undefined)).not.toBe(oldPlace);
  });

  it(`doesn't keep places beyond its capacity`, async () => {
    const lookup = new CachedPlaceLookup(1);
    const place = await lookup.getPlace('id1', undefined);
    await lookup.getPlace('id2', undefined);
    expect(await lookup.getPlace('id1', undefined)).not.toBe(place);
  });
});
